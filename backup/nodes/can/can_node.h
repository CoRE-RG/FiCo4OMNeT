...
using namespace std;
/**
 * @brief Represents a participating node in a CAN-Network
 *
 * The node is responsible for his messages and the scheduling of them.
 * Depending on the configuration of the simulation, the node takes appropriate actions upon incoming messages and the sending of messages.
 *
 */
class CAN_Node : public cSimpleModule
{
	private:
		/**
		...
		*
		*/
		long rcvdInteresting;	//Nachrichten, die für diesen Knoten interessant waren
		/**
		...
		*
		*/
		virtual void newTime(int id, simtime_t timetaken);	//Wenn eine Nachricht ankommt und die Übertragungsdauer eingepflegt wird

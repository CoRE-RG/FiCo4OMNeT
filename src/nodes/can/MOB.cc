#include "MOB.h"
	...
	//Original Stuffing bits, Payload aktiviert:
	...
	void MOB::calc_length(int bs, int percentage){
		...
		switch(bs){
			//F�r keine stuffing bits:
			case 0: length = 47 + idlength + (datalength<<3); //47 Bits ist die Laenge des Frames ohne Daten. Daten * 8 f�r bits
			break;
	...
	int MOB::binstuffbits(){
		...
		/*EV << result << " Stopfbits f�r die ID " << id << endl;
		EV << endl;*/
		return result;
	}
...
//############## 11 Bit Identifier: ############################
		if(version){
			...
			if(payload){
				...
				templength = datalength*8;	//L�nge in Bits
			...
			/*EV << "Der Daten-Frame " << id << " bin�r: " << endl;
			int binsize = binary.size();
